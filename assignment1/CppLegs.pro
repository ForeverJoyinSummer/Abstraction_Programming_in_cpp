###############################################################################
# Project file for CS101 student program
#
# @version CS101 Spring 2023 for Qt6
# @author xuehao
#   Fix some issues found in the assignments
#
...
SPL_VERSION = 2022.2
SPL_URL = https://pan.baidu.com/s/1ISzQaCm2ojfapG9ypl-ykQ?pwd=abcd
...
###############################################################################
#       Find/use installed version of cs101 lib and headers                   #
###############################################################################
...
SPL_DIR         =   $${USER_DATA_DIR}/cs101
STATIC_LIB      =   $$system_path($${SPL_DIR}/lib/libcs101.a)
...
check_lib.commands  =  $(error No CS101 library found. Install CS101 package following instructions at $${SPL_URL})
...
check_version.commands  =  $(error Cannot find version $${SPL_VERSION} of CS101 library. Install CS101 package following instructions at $${SPL_URL})
QMAKE_EXTRA_TARGETS +=  check_version
PRE_TARGETDEPS       +=  $${check_version.target}

# link against libcs101.a, add library headers to search path
# libcs101 requires libpthread, add link here
LIBS            +=  -lcs101 -lpthread
